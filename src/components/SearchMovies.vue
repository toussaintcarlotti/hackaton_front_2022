    ...
    <div class="relative mt-8">
      ...
      <div v-if="state.success" class="p-1 absolute border w-full bg-white z-50 rounded-xl border mt-2">
        <div class="py-1 px-3 rounded-xl space-y-2 " v-if="result.length>0" >
          <div class="hover:bg-gray-100 cursor-pointer py-1 px-3 rounded-xl" v-for="movie in result" :key="movie" @click="selectMovie(movie)">{{ movie.title }}</div>
        </div>
        <div v-else>Aucun résultats</div>
      </div>
    </div>
    <div class="flex flex-wrap gap-3 mt-4">
      <div class="border p-4 rounded-xl flex" v-for="movie in movies" :key="movie">
